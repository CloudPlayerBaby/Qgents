 ...
 * 发送注册邮箱验证码邮件。
 * <p>
 * 邮件包含 6 位数字验证码，验证码 10 分钟内有效，用于注册时校验邮箱真实归属。
 * 异步发送；发送失败只记录固定事件，不暴露验证码明文到日志。
 */
@Component
public class VerificationCodeMailer {
    ...
    public VerificationCodeMailer(JavaMailSender sender, @Value("${spring.mail.username}") String from) {
        this.sender = sender;
        this.from = from;
    }
    ...
    public void send(String email, String code) {
                        ...
                        <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 12px rgba(15,23,42,.06);">
                          <tr>
                            <td style="background:#2563eb;padding:28px 32px;color:#ffffff;">
                              <h1 style="margin:0;font-size:22px;font-weight:600;">验证你的 Qgents 邮箱</h1>
                            </td>
                          </tr>
                          <tr>
                            <td style="padding:32px;">
                              <p style="margin:0 0 8px;font-size:15px;color:#334155;line-height:1.7;">你正在注册 Qgents 账号，本次验证码为：</p>
                              <p align="center" style="margin:24px 0;font-size:36px;font-weight:700;letter-spacing:8px;color:#2563eb;">%s</p>
                              <p style="margin:0 0 24px;font-size:13px;color:#475569;line-height:1.7;">验证码 10 分钟内有效。如果你没有发起注册，请忽略此邮件，切勿将验证码告知他人。</p>
        ...
        try {
            ...
            helper.setFrom(from);
            helper.setTo(email);
            helper.setSubject("Qgents 注册验证码");
            helper.setText(html, true);
            sender.send(message);
        } catch (MailException e) {
            // 发送失败记录异常摘要（不含验证码），便于排查 SMTP 问题。
            log.warn("Verification code email delivery failed: {}", e.getMessage());
        } catch (Exception e) {
            // MimeMessage 构造异常视为邮件组装失败，同样不暴露验证码细节。
            log.warn("Verification code email assembly failed: {}", e.getMessage());
        }
    }
}
