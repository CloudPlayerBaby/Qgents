...
| 执行记录 | `GET /internal/v1/tool-executions/{executionId}` | status / exitCode / result / failureReason |
...
主后端会把每次 Agent Run 内完成的 Worker 工具执行摘要写入项目成员可见的 TaskRun 脱敏日志，内容包含
`executionId`、工具名、状态、退出码和脱敏 `failureReason`，不包含工具参数、文件内容、stdout 或 stderr。
...
2. 在 `node=WORKER/file.patch`（或其他 `WORKER/*`）日志项中取得 `executionId`。例如：

   ```text
   executionId=01...，tool=file.patch，status=FAILED，failureReason=FILE_PATCH_FAILED: 补丁上下文与文件不一致（第 7 行）
   ```

3. 仅由后端运维或受控诊断程序，在 Worker 内网调用：
   ...
   用 `nextCursor` 继续读取，直到不再推进。`failureReason` 在执行详情中，`/logs` 返回
