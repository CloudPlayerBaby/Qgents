...
CREATE TABLE IF NOT EXISTS
    projects (
        ...
        avatar_url TEXT NULL COMMENT '项目头像URL（OSS 公共读长期地址，可为空）',
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    failure_code VARCHAR(64) NULL COMMENT 'Task 级启动/编排失败稳定码',
    failure_reason VARCHAR(1024) NULL COMMENT 'Task 级脱敏失败说明',
    failure_retryable TINYINT(1) NULL COMMENT 'Task 级失败是否允许重试',
    failure_occurred_at DATETIME(6) NULL COMMENT 'Task 级失败发生时间（UTC）',
...
CREATE TABLE IF NOT EXISTS
    test_runs (
        ...
        started_at DATETIME(6) NULL COMMENT '开始执行时间（UTC）',
        finished_at DATETIME(6) NULL COMMENT '结束执行时间（UTC）',
        ...
        KEY idx_test_run_list (project_id, created_at, id),
...
CREATE TABLE IF NOT EXISTS
    dry_runs (
        ...
        KEY idx_dry_run_list (project_id, created_at, id),
...
CREATE TABLE IF NOT EXISTS
    notifications (
        ...
        dedupe_key VARCHAR(256) NULL COMMENT '通知幂等键；MR_PENDING 为资源ID与状态组合',
        ...
        UNIQUE KEY uk_notif_recipient_dedupe (recipient_user_id, dedupe_key),
