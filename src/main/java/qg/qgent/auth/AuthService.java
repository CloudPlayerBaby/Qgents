...
public class AuthService {
    ...
    public AuthDtos.Tokens register(AuthDtos.Register input) {
        ...
        String password = validated(rsa.decrypt(input.passwordKeyId(), input.password()));
        var user = new UserRepository.User(UuidV7.next(), email, input.displayName().trim(), null,
                passwords.encode(password), "ACTIVE");
        try {
            users.insert(user);
    ...
    public AuthDtos.Tokens login(AuthDtos.Login input, String fingerprint) {
        ...
        boolean passwordMatches = passwords.matches(plain,
                optionalUser.map(UserRepository.User::passwordHash).orElse(dummyPasswordHash));
        var user = optionalUser.orElseThrow(this::badCredentials);
        if (!"ACTIVE".equals(user.status()) || !passwordMatches)
    ...
    public AuthDtos.Tokens refresh(String raw) {
        var old = users.activeRefresh(tokens.hash(raw))
                .orElseThrow(() -> unauthorized("INVALID_REFRESH_TOKEN", "refresh token无效或已过期"));
        users.revokeRefresh(old.id());
        var user = users.findById(old.userId()).filter(u -> "ACTIVE".equals(u.status()))
                .orElseThrow(() -> unauthorized("INVALID_REFRESH_TOKEN", "用户不可用"));
        return issue(user);
    }

    @Transactional
    public void logout(UUID userId, String raw) {
        users.activeRefresh(tokens.hash(raw)).filter(t -> t.userId().equals(userId))
                .ifPresent(t -> users.revokeRefresh(t.id()));
    }
    ...
    public void requestReset(String rawEmail, String fingerprint) {
        ...
        users.findByEmail(email).filter(u -> "ACTIVE".equals(u.status())).ifPresent(user -> {
            ...
            users.insertReset(UuidV7.next(), user.id(), tokens.hash(raw), tokens.resetExpiry());
    ...
    public void reset(AuthDtos.Reset input) {
        var reset = users.activeReset(tokens.hash(input.token())).orElseThrow(
                () -> new ApiException(HttpStatus.UNPROCESSABLE_ENTITY, "INVALID_RESET_TOKEN", "重置令牌无效或已过期"));
        String password = validated(rsa.decrypt(input.passwordKeyId(), input.newPassword()));
        users.updatePassword(reset.userId(), passwords.encode(password));
        users.useAllResets(reset.userId());
        users.revokeAllRefresh(reset.userId());
    }
    ...
    public AuthDtos.UserView updateMe(UUID userId, AuthDtos.UpdateMe input) {
        if (input.displayName() == null && input.avatarUrl() == null)
    ...
    private AuthDtos.Tokens issue(UserRepository.User user) {
        ...
        users.insertRefresh(UuidV7.next(), user.id(), tokens.hash(refresh), tokens.refreshExpiry());
        return new AuthDtos.Tokens(tokens.access(user.id()), tokens.accessSeconds(), refresh,
                tokens.refreshSeconds(), view(user));
    }

    private AuthDtos.UserView view(UserRepository.User u) {
        return new AuthDtos.UserView(u.id().toString(), u.email(), u.displayName(), u.avatarUrl());
    }
    ...
    private ApiException unauthorized(String c, String m) {
        return new ApiException(HttpStatus.UNAUTHORIZED, c, m);
    }

    private ApiException conflict(String c, String m) {
