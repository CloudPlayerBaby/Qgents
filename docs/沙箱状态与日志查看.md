...
> - 前端请求客户端使用的 API：由 `src/api/client.ts` 自动拼接，并以 `credentials: include` 携带 HttpOnly 登录 Cookie；写请求同时回传 `X-XSRF-TOKEN`。开发代理模式下通常表现为 `/api/...`；生产配置可直接请求后端完整地址。
> - 后端接口契约地址：`/api/v1/...`，例如 `GET /api/v1/projects/{projectId}/task-runs/{taskRunId}/logs`。这是接口路径，不是前端页面，不能直接在地址栏打开来代替登录后的前端请求。
>
> 浏览器地址栏对同站 API 会自动携带 HttpOnly Cookie，但不能替代前端对写请求携带 CSRF header 的行为。应从前端页面操作，或在开发者工具 Network 中确认 Cookie 与 CSRF header 都已发送。
