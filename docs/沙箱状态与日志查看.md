...
`executionId`、工具名、状态、退出码、稳定 `failureCode` 和受控中文失败说明，不包含工具参数、文件内容、stdout、stderr 或 Worker 原始 `failureReason`。
...
若实际调用过 Worker，`workerExecutions` 会列出结构化的 `executionId/tool/status/failureCode/failureSummary`。没有调用 Worker 时返回空数组，但 `failure` 仍然提供主后端失败原因，因此不会再因为没有 `executionId` 而失去错误入口。
