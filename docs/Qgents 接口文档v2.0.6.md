...
|方法|路径|权限|说明|
|---|---|---|---|
|`POST`|`/auth/register`|匿名|邮箱注册平台账号（需先通过邮箱验证码校验）|
|`POST`|`/auth/register/verification-codes`|匿名|发送注册邮箱验证码（6 位数字，10 分钟有效）|
|`POST`|`/auth/login`|匿名|邮箱密码登录|
|`POST`|`/auth/refresh`|匿名|轮换 refresh token|
|`POST`|`/auth/logout`|登录用户|使当前 refresh token 失效|
|`POST`|`/auth/password-reset-requests`|匿名|发起找回密码：发送 6 位数字邮箱验证码（30 分钟有效，一次性）|
|`POST`|`/auth/password-resets`|匿名|使用邮箱验证码设置新密码|
|`GET`|`/me`|登录用户|当前账户、团队和项目角色摘要|
|`PATCH`|`/me`|登录用户|修改昵称和头像|
|`POST`|`/me/avatar/credential`|登录用户|签发头像直传凭证（OSS 未启用时 501）|
|`POST`|`/me/avatar/confirm`|登录用户|确认头像上传并返回公共读长期 URL（OSS 未启用时 501）|



**注册邮箱验证码（v2.0.6 补充）**：

注册前必须先调 `POST /auth/register/verification-codes` 获取验证码（防止假邮箱注册）：

- 请求体：`{ "email": "member@example.com" }`；响应 202 `{ "message": "验证码已发送到邮箱，10 分钟内有效" }`。
- 邮箱**已注册**返回 `409 EMAIL_ALREADY_REGISTERED`（不发送邮件）。
- 限流：同一 IP+邮箱 1 小时内最多 5 次，超出返回 `429 RATE_LIMITED`。
- 验证码为 **6 位数字**，10 分钟有效，**一次性使用**；服务端仅存 SHA-256 哈希，不落明文。
- 注册时 `verificationCode` 必填（长度固定 6）；校验失败返回 `422 INVALID_VERIFICATION_CODE`（不区分过期/不存在/已使用，避免枚举）。
...
- `POST /auth/password-reset-requests`（body `{ "email": "member@example.com" }`）→ 向邮箱发送 **6 位数字验证码**（30 分钟有效、一次性；服务端仅存 SHA-256 哈希）。未注册邮箱同样返回 202（规避枚举）；限流同一 IP+邮箱 1 小时 3 次。
- `POST /auth/password-resets`：body 为 `{ "token": "483920", "newPassword": "...", "passwordKeyId": "..." }`——`token` 即邮箱验证码；校验失败返回 `422 INVALID_RESET_TOKEN`。



**头像上传流程**：用户选本地图片 → `POST /me/avatar/credential`（body `{mediaType, sizeBytes}`，`mediaType` 必须 `image/*`、大小 ≤5MB）签发直传凭证 → 客户端用返回的 `uploadUrl` 直传 OSS → `POST /me/avatar/confirm`（body `{objectKey}` 原样回传）确认识别。对象键由服务端生成 `avatars/{userId}/{uuid}.{ext}`（扩展名白名单 jpg/jpeg/png/webp），客户端不得自造；confirm 校验对象属于当前用户且已真实上传，写入 `users.avatar_url` 并返回**长期稳定、公共可读**的头像 URL（各上下文通用，不依赖项目鉴权）。OSS 未启用（本地/CI）时两端点返回 `501 AVATAR_STORAGE_NOT_CONFIGURED`，前端应隐藏/提示「头像上传暂不可用」。头像桶（或 `avatars/` 前缀）需设为公共可读，并配置环境变量 `ALIYUN_OSS_PUBLIC_BASE_URL` 作为公共读基础 URL。`PATCH /me` 的 `avatarUrl` 仍接受任意 http\(s\) URL（兼容既有客户端），新增头像上传是推荐入口。
...
|方法|路径|权限|说明|
|---|---|---|---|
|`POST`|`/teams`|登录用户|创建团队，创建者成为 `TEAM_OWNER`（body 可带 `avatarUrl`）|
|`GET`|`/teams`|登录用户|获取我加入的团队（响应含 `avatarUrl`）|
|`GET`/`PATCH`|`/teams/{teamId}`|团队成员/Team Owner|获取/修改团队资料（PATCH 支持 `name`/`description`/`avatarUrl` 任一单独更新，null 保留原值、空串清空对应字段）|
|`POST`|`/teams/{teamId}/avatar/credential`|团队成员|签发团队头像直传凭证（OSS 未启用时 501）|
|`POST`|`/teams/{teamId}/avatar/confirm`|团队成员|确认团队头像上传并返回公共读 URL|
|`GET`|`/teams/{teamId}/members`|团队成员|团队成员列表（响应含 `avatarUrl`）|
...
|`GET`|`/teams/by-last-activity`|登录用户|获取我加入的团队，按最后活跃时间倒序（不分页；团队最后活跃 = 该团队下所有项目最后活跃的最大值，旗下任何项目都无消息时为 `null` 沉底；响应 `data[].lastActivityAt` 为 ISO8601 UTC）|
...
**团队头像上传流程（v2.0.6 补充）**：与用户/Agent 头像同模式——`POST /teams/{teamId}/avatar/credential`（body `{mediaType, sizeBytes}`，`mediaType` 必须 `image/*`、大小 ≤5MB）签发直传凭证（对象键 `teams/{teamId}/{uuid}.{ext}`）→ 客户端用 `uploadUrl` 直传 OSS → `POST /teams/{teamId}/avatar/confirm`（body `{objectKey}`）返回公共读 URL。创建团队页在创建成功后用返回的 `teamId` 走该流程，再把 URL 随 `PATCH /teams/{teamId}`（`avatarUrl`）回写；`TeamResponse`/`GET /teams`/`GET /teams/{teamId}`/`GET /teams/{teamId}/members` 均返回 `avatarUrl`（成员列表为 `users.avatar_url`）。OSS 未启用时两端点返回 `501 AVATAR_STORAGE_NOT_CONFIGURED`。
...
|`GET`|`/teams/{teamId}/projects`|团队成员|仅返回我有权限访问的项目（响应含 `avatarUrl`）|
|`GET`|`/teams/{teamId}/projects/by-last-activity`|团队成员|返回某团队下当前用户可见的项目，按最后活跃时间倒序（不分页；项目最后活跃 = 其下所有群最近消息时间的最大值，任何群都无消息时为 `null` 沉底；响应 `data[].lastActivityAt` 为 ISO8601 UTC）|
|`GET`/`PATCH`|`/projects/{projectId}`|项目成员/`PROJECT_ADMIN`|获取/修改项目资料（PATCH 支持 `name`/`description`/`avatarUrl` 任一单独更新，全缺返回 `422 INVALID_PROJECT_OPERATION`）|
|`POST`|`/projects/{projectId}/avatar/credential`|项目成员|签发项目头像直传凭证（OSS 未启用时 501）|
|`POST`|`/projects/{projectId}/avatar/confirm`|项目成员|确认项目头像上传并返回公共读 URL（不写字段，前端随 PATCH 回写）|
...
**项目头像上传流程（v2.0.6 补充）**：`POST /projects/{projectId}/avatar/credential`（body `{mediaType, sizeBytes}`，`mediaType` 必须 `image/*`、大小 ≤5MB）签发直传凭证（对象键 `projects/{projectId}/{uuid}.{ext}`）→ 客户端用 `uploadUrl` 直传 OSS → `POST /projects/{projectId}/avatar/confirm`（body `{objectKey}`）返回公共读 URL → 随 `PATCH /projects/{projectId}`（`avatarUrl`）回写。入口在项目群聊/主群聊天页右上角「群聊设置」；`ProjectResponse` 列表/详情/by-last-activity 均返回 `avatarUrl`。OSS 未启用时两端点返回 `501 AVATAR_STORAGE_NOT_CONFIGURED`。
...
|`POST`<br>|`/projects/{projectId}/repositories`|Project Admin|绑定团队已授权仓库|
...
|参数|类型|必填|说明|
|---|---|---|---|
|`repositoryId`|UUID|否|`project_repositories.id`。传入不属于当前项目的 ID 返回 `404 REPOSITORY_NOT_FOUND`。|
|`requirementGroupId`|UUID|否|REQUIREMENT Group UUID。传入不存在、非当前项目或非 REQUIREMENT 的 Group 返回 `404 REQUIREMENT_GROUP_NOT_FOUND`。未传返回全部；传入时仅返回存在关联 Task 属于该群的工作分支。|
|`cursor`|string|否|上一页返回的 `page.nextCursor`。失效或格式非法返回 `400 INVALID_CURSOR`。|
...
\*、`continuationOfTaskId=源Task` 的增量任务，仓库范围由该 Workspace 继承。客户端**不得**提交 \`workspa
...
|---|---|---|
|`id`|string|通知 id|
|`kind`|string|`TASK_COMPLETED` / `TASK_FAILED` / `AGENT_INPUT_REQUIRED` / `DELIVERABLE_PENDING` / `MR_PENDING` / `INVITED` / `TEAM_JOINED` / `PROJECT_ADDED` / `MESSAGE_MENTION`|
|`title`|string|一行标题|
...
**免审批规则（v2.0.6 补充）：** Project Admin **手动创建**的 Memory 直接 `APPROVED` 上架（免审批）；普通成员手动创建为 `DRAFT`。AI 沉淀草稿（`POST /memories/drafts`）始终为 `DRAFT`，任何人均需审核。
...
|`POST`|`/projects/{projectId}/memories/drafts`|项目成员|按群自动检索最近聊天，AI 生成草稿（`DRAFT`，AI 不得直接批准）|
...
|方法|路径|权限|说明|
|---|---|---|---|
|`GET`|`/teams/{teamId}/agents`|团队成员|查询系统 Agent、本人私有 Agent、待审核 Agent（仅创建者与 Team Owner 可见）和团队已发布 Agent|
...
|`POST`|`/teams/{teamId}/agents/{agentId}/publish`|创建者|**提交发布审核**：`PRIVATE` → `PENDING`（不再直接发布为 `TEAM`）|
|`POST`|`/teams/{teamId}/agents/{agentId}/approve`|Team Owner|批准发布：`PENDING` → `TEAM`（团队共享；批准后不可再变回私有，只能归档）|
|`POST`|`/teams/{teamId}/agents/{agentId}/reject`|Team Owner|拒绝发布：`PENDING` → `PRIVATE`（body 可带 `reason` 拒绝原因，仅创建者可见；创建者可修正后重新提交）|
|`POST`|`/teams/{teamId}/agents/{agentId}/unpublish`|—|**已废弃**：已发布 `TEAM` 不可收回为私有，返回 `409 AGENT_UNPUBLISH_DISALLOWED`（只能归档）|
...
|`POST`|`/teams/{teamId}/agents/{agentId}/publish`|创建者|提交发布审核（`PRIVATE+ACTIVE` → `PENDING+ACTIVE`）；需 `Idempotency-Key`|
|`POST`|`/teams/{teamId}/agents/{agentId}/approve`|Team Owner|批准发布（`PENDING+ACTIVE` → `TEAM+ACTIVE`）；需 `Idempotency-Key`|
|`POST`|`/teams/{teamId}/agents/{agentId}/reject`|Team Owner|拒绝发布（`PENDING+ACTIVE` → `PRIVATE+ACTIVE`，body 可带 `{"reason": "..."}`）；需 `Idempotency-Key`|
|`POST`|`/teams/{teamId}/agents/{agentId}/unpublish`|—|已废弃：`409 AGENT_UNPUBLISH_DISALLOWED`（TEAM 不可收回为私有，只能归档）|
|`POST`|`/teams/{teamId}/agents/{agentId}/archive`|创建者或 Team Owner|归档（`PRIVATE/PENDING/TEAM+ACTIVE` → `ARCHIVED`），已运行任务不受影响；需 `Idempotency-Key`|
...
- `visibility` 取值：`PRIVATE` / `PENDING` / `TEAM`；`PENDING` 仅创建者与 Team Owner 可见（列表/详情）。
- `AgentResponse` 新增审核字段：`reviewReason`（拒绝原因，**仅创建者可见**）、`reviewedBy`（审核人 ID）、`reviewedAt`（审核时间 ISO8601 UTC）。
- 发布审核通过交付中心 `AGENT` 类型聚合管理（见 §20.5），审批/拒绝复用 `approve`/`reject` 端点。
...
|规则|字段|类型|取值|
|---|---|---|---|
|用户显式指定优先；未指定时由 Planner 判定；Planner 缺失、非法或不确定时由服务端硬规则兜底。非法值返回 `400 INVALID_DELIVERY_MODE`。续作 Task 沿用源 Task 的已定型模式。|`deliveryMode`|string，可选|`DIFF_FIRST` / `MR_FIRST`|
|由服务端写入（Planner `scaleReason` 或规则依据），客户端不得提交或覆盖。|`deliveryReason`|string，只读，可空|判定理由|
...
|状态|含义|
|---|---|
|`WAITING_DIFF_CONFIRMATION`|仅 `DIFF_FIRST` 使用：最终 Diff 已生成，等待用户确认或拒绝。|
|`DELIVERING`|交付已获授权，正在逐仓库执行真实 Commit、Push 与 PR 创建。此状态不代表 PR 已创建。|
|`SUCCEEDED`|所有目标仓库均已真实创建 PR；PR 是否通过质量门禁、是否被合并由 MR 资源表达。|
|`DELIVERY_FAILED`|至少一个仓库交付失败；已成功仓库不回滚，允许重试失败仓库。|
...
#### 12\.1\.1 MR\_FIRST 开始交付事件

`delivery.started` 是项目 SSE 事件，表示 MR\_FIRST 的交付意图已与 Task 状态、最终 Diff 快照和交付租约一并落库。它仅用于前端刷新展示；前端不得将 SSE 当作命令通道。
...
|字段|必填|说明|
|---|---|---|
|`projectId`、`taskId`、`reviewBatchId`、`deliveryMode`、`operationId`|是|本次已持久化交付意图的脱敏标识。`operationId` 仅用于前端关联同一次交付状态，不代表凭证或可由客户端调用的操作令牌。|
|`reason`|否|`deliveryReason` 非空时返回。|
...
- 端点：`GET /api/v1/teams/{teamId}/events`（**团队成员**可订阅；`Content-Type: text/event-stream`）
...
- 端点：`GET /api/v1/notifications/events`（**当前登录用户**维度；`Content-Type: text/event-stream`）
...
```JSON
{
  ...
  "sequence": 12,
  "entryType": "EXECUTION",
  "node": "WORKER/file.patch",
  "content": "executionId=worker-execution-uuid，tool=file.patch，status=FAILED，failureReason=FILE_PATCH_FAILED: 补丁上下文与文件不一致（第 7 行）",
  "timestamp": "2026-08-12T10:00:00Z"
}
```

日志分页游标取上页最后一条的 `sequence`；`node` 为产生日志的执行节点名，单节点运行为空。`WORKER/*` 条目只返回 Worker 工具执行的脱敏诊断摘要：`executionId/tool/status/exitCode/failureReason`，不返回工具参数、Patch、文件内容、stdout 或 stderr。`executionId` 仅供后端运维通过 Worker 内网接口定位完整执行记录，前端不得直接调用 `/internal/v1/**`。Worker 内网接口必须携带独立服务令牌并在未配置令牌时 fail-closed；服务令牌不得进入浏览器、App、TaskRun 日志或 SSE。完整日志查看流程见《沙箱状态与日志查看》。如需步骤级清单回看，`TaskRun` 详情可附 `steps` 数组（节点状态 `PENDING/RUNNING/PASSED/FAILED/SKIPPED/CANCELLED`，含 `node`/`status`/`startedAt`/`finishedAt`/`durationMs`/可选 `errorCode`），由执行服务提供。
...
`reviewStatus=ACCEPTED` 的统一语义是“该批次已获准进入交付”；**谁授权交付**由只读字段 `confirmationSource` 表达：



|交付模式|批次状态|`confirmationSource`|用户体验|
|---|---|---|---|
|`DIFF_FIRST`|初始为 `PENDING_CONFIRMATION`；用户确认后为 `ACCEPTED`|`USER`|展示 Diff 确认/拒绝操作。|
|`MR_FIRST`|创建即为 `ACCEPTED`|`SYSTEM`|不展示 Diff 确认/拒绝操作，展示“自动交付”与 MR 交付进度。|



`SYSTEM` 表示系统依据 MR\_FIRST 规则自动授权，不表示用户已经确认 Diff。前端、群聊卡片、审计记录和文案不得将 `confirmationSource=SYSTEM` 展示为“用户已确认”。



系统只允许 MR\_FIRST 内部流程写入 `SYSTEM`；客户端请求体不包含该字段。历史批次和 DIFF\_FIRST 用户确认批次均为 `USER`。
...
- **适用范围**：仅 `reviewStatus=PENDING_CONFIRMATION` 且 `confirmationSource=USER` 的 DIFF\_FIRST 批次。

- **响应**：`200 OK`，返回 `reviewStatus=ACCEPTED`、`confirmationSource=USER` 的 `DiffReviewBatchResponse`。服务端完成全部仓库预检后进入 `DELIVERING`，再逐仓库交付。

- **错误**：MR\_FIRST 的 `ACCEPTED + SYSTEM` 批次不接受用户确认，返回 `409 DIFF_REVIEW_NOT_DECIDABLE`。
...
- **适用范围**：仅 `PENDING_CONFIRMATION + USER` 的 DIFF\_FIRST 批次。

- **响应**：`200 OK`，返回 `reviewStatus=REJECTED` 的 `DiffReviewBatchResponse`。拒绝后不进行 Commit、Push 或 PR 创建。

- **错误**：MR\_FIRST 的 `ACCEPTED + SYSTEM` 批次不接受用户拒绝，返回 `409 DIFF_REVIEW_NOT_DECIDABLE`。
...
|字段|取值|前端含义|
|---|---|---|
|`reviewStatus`|`PENDING_CONFIRMATION`、`ACCEPTED`、`REJECTED`|批次是否已获准进入交付。`ACCEPTED` 不等同于“用户已确认”。|
|`confirmationSource`|`USER`、`SYSTEM`|授权来源。`USER` 表示用户确认；`SYSTEM` 表示 MR\_FIRST 自动授权。|
|`deliveryStatus`|`NOT_STARTED`、`DELIVERING`、`DELIVERED`、`PARTIALLY_DELIVERED`、`FAILED`|总体交付结果。|
|单仓库持久化状态|`NOT_STARTED`、`COMMITTED`、`MR_CREATED`、`FAILED`|通过 `repositoryDeliveries[]` 返回，用于交付进度与重试。|
...
`confirm` 成功表示 `confirmationSource=USER` 的用户已接受整个快照；MR\_FIRST 的自动交付以 `confirmationSource=SYSTEM` 表示，不经过 `confirm` 接口。无论授权来源是什么，`ACCEPTED` 都不表示所有 PR 已创建；前端必须结合 `deliveryStatus`、`repositoryDeliveries[]` 和后续 SSE 展示真实交付结果。
...
|事件|payload 字段|推荐刷新资源|
|---|---|---|
|`delivery.started`|`projectId`、`taskId`、`reviewBatchId`、`deliveryMode`、`operationId`，可选 `reason`|Task、Task Diff Review|
...
|`diff-review.skipped`|`projectId`、`taskId`、`reason`|Task|
...
|`keyword`|string|按不区分大小写包含匹配 `displayCode`/`title`/`requirement`/需求群名/创建人/绑定仓库展示名与全名（v2.0.6 补充；超 100 字符返回 422）|
...
|`keyword`|string|否|按不区分大小写包含匹配 title/summary/resourceId/来源任务展示码与标题/创建人/提交人/CODE 仓库名/MEMORY·SKILL 摘要字段（v2.0.6 补充）|
...
|字段|类型|说明|
|---|---|---|
|`id` / `resourceId`|string|资源 ID：CODE=批次 ID、MEMORY=memoryId、SKILL=skillId、AGENT=agentId|
|`projectId`|string|项目 ID|
|`resourceType`|string|`CODE` / `MEMORY` / `SKILL` / `AGENT`|
|`title`|string|展示标题|
|`summary`|string?|脱敏摘要（≤200 字符）|
|`version`|string?|当前恒为 null|
|`displayStatus`|string|后端统一派生的展示状态（前端不维护映射）|
|`resourceStatus`|string|真实资源状态（reviewStatus / Memory\.status / Skill\.status / Agent\.visibility）|
...
**AGENT 专属字段（v2.0.6）**：`role`（角色标签）、`descriptionExcerpt`（用途描述摘要，创建者或 Team Owner 可见）、`isDefault`（恒 false，系统预置 Agent 不进交付中心）。**不返回 `prompt` 全文**（敏感）。

- 进入范围：`PENDING`（待审核）/`TEAM`（已批准共享）/`ARCHIVED`（已归档）；`PRIVATE`（未提交审核）与系统预置 Agent 不进入。
- 展示状态映射：`PENDING`→`PENDING_REVIEW`、`TEAM`→`ACCEPTED`、`ARCHIVED`→`ARCHIVED`。
- `openTarget = { kind: "AGENT", agentId }`。
- 操作能力：`canApprove`/`canReject` = Team Owner 且 PENDING；`canArchive` = 创建者或 Team Owner 且 TEAM；`canSubmitReview` 恒 false（提交审核走 `POST /teams/{teamId}/agents/{agentId}/publish`，Agent 管理页，不进交付中心）。
...
- `countsByType` 恒含 `CODE/MEMORY/SKILL/AGENT` 四 key（值为 0 也返回）；`countsByStatus` 恒含 8 个正式大写枚举 key（`DRAFT/PENDING_REVIEW/PROCESSING/ACCEPTED/REJECTED/DELIVERED/FAILED/ARCHIVED`）；
...
**openTarget 五态**（`resourceId` 不再被多义解释为 Diff 或批次）：
...
type DeliveryOpenTarget =
  ...
  | { kind: "AGENT"; agentId: string };   // v2.0.6：自定义 Agent 发布审核项
```
...
|AGENT|visibility=PENDING / TEAM / ARCHIVED|`PENDING_REVIEW` / `ACCEPTED` / `ARCHIVED`|
...
PENDING\_CONFIRMATION \+ confirmationSource=USER；MR\_FIRST 的 ACCEPTED \+ SYSTEM 不显示确认/拒绝操作；retry\-delivery 仅接受后交付未完成（`PARTIALLY_DELIVERED`/`FAILED`）；
...
|---|---|---|---|
|`groupId`|UUID|否|按需求群筛选（CODE 按 Task 群来源、MEMORY 按来源消息群匹配；SKILL 无来源不匹配）|
|`type`|string|否|资源类型：`CODE` / `MEMORY` / `SKILL`；省略导出全部三类|
|`status`|string|否|按展示状态 `displayStatus` 筛选（枚举见 §20\\\.3）|
|`repositoryId`|UUID|否|按项目仓库绑定 ID 筛选（仅 CODE 匹配）|
|`createdBy`|UUID|否|按创建者筛选|
...
|列|来源|说明|
|---|---|---|
|类型|`resourceType`|`CODE` / `MEMORY` / `SKILL`|
|标题|`title`|展示标题|
|摘要|`summary`|≤200 字符脱敏摘要|
|展示状态|`displayStatus`|后端派生（§20\\\.3）|
|资源状态|`resourceStatus`|真实资源状态|
|需求群|`requirementGroup.name`|无群来源时为空|
|来源任务编号|`source.taskDisplayCode`|仅 CODE|
|来源任务标题|`source.taskTitle`|仅 CODE|
|创建人|`creator.displayName`||
|审核人|`reviewer.displayName`||
|驳回原因|`reviewReason`|未拒绝时为空|
|创建时间 / 审核时间 / 更新时间|`createdAt` / `reviewedAt` / `updatedAt`|ISO8601 UTC|
|变更文件数 / 新增行数 / 删除行数|`filesChanged` / `additions` / `deletions`|仅 CODE，其他类型为空|
|仓库|`repositories[].name`|仅 CODE，分号连接，去重|
...
|`newRepository`|object|自动新建一个 GitHub 仓库并绑定；与 `repositoryIds` 互斥（二选一）|

自动新建仓库：后端建仓时固定传 auto\_init: true，GitHub 会自动生成初始 README 提交并据此建立默认分支（一般是 main）。建仓返回的元数据中带有真实 defaultBranch，并直接写入本地仓库镜像，因此新建仓库立即可绑定、可克隆，不会出现空仓库缺默认分支的问题。前端无需额外处理。
...
|字段|类型|说明|
|---|---|---|
|`name`|string|仓库名，必填，符合 GitHub 命名约束|
|`description`|string|仓库描述，可选|
|`isPrivate`|boolean|是否私有，默认 `true`|
|`installationId`|string $UUID$|建仓使用的安装记录 ID，团队仅一个 ACTIVE 安装时可省略|
|`displayName`|string|项目内显示名称，默认取仓库名|
...
|方法|路径|说明|
|---|---|---|
|`POST`|`/api/v1/projects/{projectId}/dry-runs/{dryRunId}/cq-approvals`|对通过且未失效的 Dry Run 给出 CQ\+1|
|`POST`|`/api/v1/projects/{projectId}/dry-runs/{dryRunId}/cq-rejections`|拒绝 CQ，`reason` 必填|
...
TASK\_STATUS 和 DIFF 是消息类型；task\-card\-\{taskId\} 和 diff\-card\-\{taskId\} 是服务端内部用于定位消息的 clientMessageId，不是消息类型。

### 23\.2 卡片唯一定位


服务端使用 messages\.client\_message\_id 定位自动化卡片：
...
6. 同一个 Task 不得因为状态变化重复创建 TASK\_STATUS 或 DIFF 消息。

### 23\.3 TASK\_STATUS 任务状态卡

TASK\_STATUS 用于展示 Task 的完整执行状态、Planner 计划和 TaskStep 快照。
...
- status：使用 Task 当前状态，例如 PLANNING、PENDING、RUNNING、WAITING\_DIFF\_CONFIRMATION、DELIVERING、SUCCEEDED、DELIVERY\_FAILED、FAILED、CANCELLED。
...
5. Task 进入等待确认、交付中、成功、失败或取消状态时，继续更新同一张 TASK\_STATUS 消息。

6. TaskStep、TaskRun 和 Task 的数据库记录仍然是执行事实的权威来源，消息卡片只是用户展示快照。

### 23\.4 DIFF 卡

DIFF 消息用于展示 Task 级 Diff 审核和交付状态，不将 Diff 内容放入 TASK\_STATUS 卡。
...
- reviewStatus 使用 PENDING\_CONFIRMATION、ACCEPTED、REJECTED。

- deliveryStatus 使用 NOT\_STARTED、COMMITTED、PUSHED、MR\_CREATED、DELIVERY\_FAILED。
...
当已有 TASK\_STATUS 或 DIFF 消息的 content 被更新时，服务端发布：
...
前端不得根据消息连续性进行聚合，也不得把普通用户消息或其他 Agent 消息合并进 TASK\_STATUS 或 DIFF 卡。
...
SYSTEM 降级只表示卡片发送身份缺失，不改变 TASK\_STATUS 或 DIFF 的消息类型，也不改变 Task、Diff、Commit、Push 或 MR 的真实状态。
...
`GET /projects/{projectId}` 返回 `ApiResponse<ProjectResponse>`，其中 `data.role` 是当前认证用户在该项目的**有效角色**，取值为 `PROJECT_ADMIN` 或 `PROJECT_MEMBER`。
...
`ProjectResponse` 另含 `repositoryCount`（v2.0.6 补充）：该项目**生效（ACTIVE）仓库绑定数**，用于项目卡/详情展示「绑定 N 个仓库」，避免前端逐卡 N+1 查询仓库列表；`GET /teams/{teamId}/projects` 列表项与 `GET /projects/{projectId}` 详情均返回。
...
## 27. 可靠消息同步与移动端推送增量契约（2026-08-23）
...
|方法|路径|权限|说明|
|---|---|---|---|
|`POST`|`/api/v1/push/devices`|登录用户|注册/刷新 Android 或 iOS 的 FCM Token；必须携带 `Idempotency-Key`|
|`DELETE`|`/api/v1/push/devices/{installationId}`|登录用户|幂等停用当前用户的安装实例；必须携带 `Idempotency-Key`|
...
## 28. Dry Run 执行与重试增量契约（2026-08-24）

Dry Run 是固定 `headCommit + targetCommit` 上的合并预演；创建受理成功仅代表 `202 QUEUED`，不代表测试通过、MR 已创建或已合并。

### 28.1 报告与执行事实

`GET /api/v1/projects/{projectId}/dry-runs/{dryRunId}/report` 新增冻结上下文和执行元数据：`headCommit`、`targetBranch`、`targetCommit`、`attemptCount`、`createdAt`、`updatedAt`。`report` 中逐 Testset 结果包含 `testsetId/status/exitCode/durationMs/failureCode/message`；`message` 是脱敏、限长摘要，不返回原始 stdout/stderr、命令、凭据或宿主机路径。

合并冲突固定为 `FAILED`，报告含 `failureCode=GIT_MERGE_CONFLICT`、冲突文件以及 `tests={status:SKIPPED, reason:MERGE_CONFLICT, results:[]}`。测试超时的顶层失败码为 `DRY_RUN_TIMEOUT`；普通门禁失败为 `TESTSET_FAILED`。其他稳定码包括 `DRY_RUN_CONTEXT_MISMATCH`、`DRY_RUN_TEST_CONTEXT_MISMATCH`、`GIT_BASE_REF_NOT_FOUND`、`SANDBOX_WORKER_UNAVAILABLE`、`TESTSET_DEFINITION_INVALID`。

Worker 必须回传并由主后端校验固定的 source/target 上下文；任何不一致均不能写入 `PASSED`。`tests` 汇总还保留 `resolvedHeadCommit`、`resolvedSourceCommit`、`resolvedTargetCommit`。

### 28.2 重试

|方法|路径|权限|说明|
|---|---|---|---|
|`POST`|`/api/v1/projects/{projectId}/dry-runs/{dryRunId}/retries`|Project Member|仅为可重试基础设施失败创建新的 `QUEUED` Dry Run；需 `Idempotency-Key`|

可重试原因仅限 `SANDBOX_WORKER_UNAVAILABLE`、`SANDBOX_WORKER_ERROR`、`GITHUB_API_UNAVAILABLE`、`GIT_STORE_FETCH_FAILED`、`DRY_RUN_TIMEOUT`。新记录固定复制原 source SHA、target SHA 和 Testset snapshot，并保存 `retryOfDryRunId`、`retryReasonCode`；原失败报告不可覆盖。最多允许三层续跑，达到上限返回 `409 DRY_RUN_RETRY_EXHAUSTED`；已有进行中续跑返回 `409 DRY_RUN_RETRY_IN_PROGRESS`；其他失败类型返回 `409 DRY_RUN_RETRY_NOT_ALLOWED`。

后续 Agent 续跑只接收结构化 `RetryContext`：`failureCode`、限长脱敏的 `failureSummary`、`failures`、`modifiedFiles`、`instruction`。它不携带原始命令输出、Token、密码、私钥、环境变量或宿主机路径。

### 28.3 事件与 MR_FIRST

`dry-run.updated` payload 固定包含 `projectId`、`dryRunId`、`repositoryId`、可选 `taskId`、`status`、`headCommit`、`targetBranch`、`targetCommit` 和 `timestamp`。客户端收到事件后必须重新请求报告，不能仅靠事件推断门禁结果。

MR_FIRST 的任意仓库收到 CQ+1 事件时，服务端会重新校验 Task 的每个 Workspace repository 都已在当前 head/target 上完成 `PASSED` Dry Run 与独立 CQ+1；全部满足后才逐仓库调用幂等 MR 创建。

### 28.4 前端接口映射

Dry Run 相关接口均使用统一 `ApiResponse` 包装，前端读取 `response.data`。创建接口返回 `202`，只表示任务已进入 `QUEUED`，不能据此显示“通过”或“MR 已创建”。请求字段保持如下：

```JSON
{
  "repositoryId": "project-repository-uuid",
  "sourceRef": "feature/task-branch-or-sha",
  "targetBranch": "main",
  "taskId": "task-uuid"
}
```

`taskId` 可选；`targetCommit` 不由前端提交，而由后端在受理时刷新目标分支并固定。前端至少应按以下结构建模（后端未返回的字段不要自行补造）：

```TypeScript
type DryRunStatus = 'QUEUED' | 'RUNNING' | 'PASSED' | 'FAILED' | 'CANCELLED'
...
interface DryRun {
  ...
  status: DryRunStatus
  report: DryRunExecutionReport | null
  createdBy: string | null
  createdAt: string
}

interface DryRunExecutionReport {
  headCommit: string | null
  targetCommit: string | null
  ...
  message: string | null
  tests: {
    status: 'PASSED' | 'FAILED' | 'NOT_REQUIRED' | 'SKIPPED'
    reason?: 'MERGE_CONFLICT' | null
    resolvedHeadCommit?: string | null
    resolvedSourceCommit?: string | null
    resolvedTargetCommit?: string | null
    results: DryRunTestsetResult[]
  } | null
}
...
报告内 `tests.results` 的每项使用 `DryRunTestsetResult`；合并冲突时 `tests.status=SKIPPED`、`tests.reason=MERGE_CONFLICT` 且 `results=[]`。`message` 只用于展示脱敏、限长摘要，不是原始日志。

### 28.5 前端重试调用

|方法|路径|请求体|成功响应|权限|
|---|---|---|---|---|
|`POST`|`/api/v1/projects/{projectId}/dry-runs/{dryRunId}/retries`|无|`202`，`data` 为新的 Dry Run 记录|Project Member|

- 必须携带新的 `Idempotency-Key`；同一次用户点击的网络重试复用同一个 key，不同点击生成新的 key。
- 成功后使用响应中的**新** `dryRun.id` 更新路由或 Query 缓存，并重新查询 `GET .../dry-runs/{newDryRunId}/report`；不得继续轮询原失败 ID 以等待新运行。
- 原 Dry Run 报告不可编辑、不可覆盖。重试血缘由服务端持久化；当前响应未返回 `retryOfDryRunId` 时，前端不得根据 ID 或时间自行推断血缘关系。
- `409 DRY_RUN_RETRY_NOT_ALLOWED` 隐藏重试按钮；`409 DRY_RUN_RETRY_EXHAUSTED` 展示“已达到重试上限”；`409 DRY_RUN_RETRY_IN_PROGRESS` 显示“已有重试正在执行”并等待 `dry-run.updated`。当前错误体不返回已有子运行 ID，前端不得从来源报告或时间顺序猜测该 ID。
- `400 IDEMPOTENCY_KEY_REQUIRED` 和 `409 IDEMPOTENCY_KEY_REUSED` 属于客户端请求错误，应按通用幂等错误处理，不自动生成第二个业务重试。
- 当前没有 `GET /dry-runs` 历史列表接口。前端应把正常 `202` 响应中的新 ID 写入现有本机会话历史；不要调用未定义的列表接口，也不要把本地历史当作服务端权威记录。

### 28.6 状态、上下文与失败码展示

前端只展示服务端状态：`QUEUED`=排队中、`RUNNING`=运行中、`PASSED`=通过、`FAILED`=失败、`CANCELLED`=已取消。不得把 `QUEUED`/`RUNNING` 渲染为通过，也不得自行增加顶层 `CONFLICT` 状态；冲突通过 `failureCode=GIT_MERGE_CONFLICT` 或 `tests.reason=MERGE_CONFLICT` 展示。

报告页必须同时显示冻结上下文 `headCommit`、`targetBranch`、`targetCommit`、`attemptCount` 和 `updatedAt`。上下文应按 SHA 原文展示或截断展示并支持复制，不能用当前分支最新 SHA 替换报告中的值。

|`failureCode`|前端提示|显示重试|
|---|---|---|
|`GIT_MERGE_CONFLICT`|源提交与目标提交存在冲突，请解决冲突后重新运行|否|
|`TESTSET_FAILED`|Testset 未通过，请查看逐项结果|否|
|`DRY_RUN_TIMEOUT`|Dry Run 执行超时|是|
|`SANDBOX_WORKER_UNAVAILABLE` / `SANDBOX_WORKER_ERROR`|执行环境暂时不可用|是|
|`GITHUB_API_UNAVAILABLE`|GitHub 服务暂时不可用|是|
|`GIT_STORE_FETCH_FAILED`|仓库同步失败|是|
|`DRY_RUN_CONTEXT_MISMATCH` / `DRY_RUN_TEST_CONTEXT_MISMATCH`|执行上下文已变化，请重新发起 Dry Run|否|
|`GIT_BASE_REF_NOT_FOUND` / `TESTSET_DEFINITION_INVALID`|Dry Run 配置无效，请修正配置后重新发起|否|

只有表中标记为“是”的基础设施失败才显示重试按钮。失败摘要、Testset `message` 和提示文案不得包含原始 stdout/stderr、命令、Token、密码、私钥、环境变量或宿主机路径。

### 28.7 SSE 与缓存一致性

`dry-run.updated` 事件 payload：
...
- SSE 事件只用于使 `dry-runs` 列表、当前报告和关联 Preflight Query 失效；收到事件后必须重新 GET 报告。
- 不使用事件 payload 拼接完整报告，也不以事件顺序代替服务端状态；重复或乱序事件必须保持幂等。
- SSE 断线重连成功后，前端应主动刷新当前报告；项目 SSE 的 `Last-Event-ID` 过期收到 `409 EVENT_CURSOR_EXPIRED` 时，重新建立连接并刷新 REST 数据。
- 事件解析类型必须包含 `targetCommit`，但前端不得据此自行判定门禁或 MR 状态。

### 28.8 MR_FIRST 与多仓库页面

MR_FIRST 页面只能依赖重新查询的 Task、每仓库 Preflight、Dry Run 和 MR 详情：

- 任一仓库没有 `PASSED` Dry Run，不得显示“MR 已创建”。
- 任一仓库缺 CQ+1，显示等待 CQ+1；一个仓库通过不能推导其他仓库通过。
- 多仓库必须按仓库分别展示 `headCommit/targetCommit`、Dry Run、CQ 和 MR 状态。
- “MR 已创建”不等于“MR 已合并”；合并状态必须读取真实 MR 详情。

### 28.9 前端验收清单

- 覆盖 `QUEUED -> RUNNING -> PASSED/FAILED` 的状态渲染和刷新。
- 报告展示 source/target SHA、执行次数、更新时间，以及逐 Testset 的 `message` 和 `failureCode`。
